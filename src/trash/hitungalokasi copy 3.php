...
foreach ($pre_allocations as $mix_cust => $entries) {
    ...
    // Tambahkan limit maksimum 25 SKU per MIXCUST
    usort($entries, fn($a, $b) => $b['alloc_qty'] <=> $a['alloc_qty']);
...
$_SESSION['final_allocation'] = $final_result;
// echo json_encode(['status' => 'success', 'data' => $final_result]);
