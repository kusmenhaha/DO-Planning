...
foreach ($cmo_processed_data as &$data) {
    ...
    if ($remaining_volume_needed_for_mixcust > 0 && $current_sku_count < $MaxCMO) {
        if ($remaining_otd_cmo_for_this_stage >= $minCarton && $remaining_stock_for_this_stage >= $minCarton) {
            ...
            if ($cmo_otd >= $rangeFullCMO) { // Menggunakan $cmo_otd asli untuk filter ini
                ...
                // Untuk SKU yang tidak masuk kriteria full CMO, alokasikan sisa dengan persentase dari OTD CMO asli * MaxCMO
                // Asumsi $percentageForNonFullCMO adalah persentase (misal 0.5 untuk 50%)
                // Pastikan $percentageForNonFullCMO telah didefinisikan sebelumnya.
                $max_qty_based_on_percentage = $cmo_otd * $percentageForNonFullCMO; 
                $qty_to_allocate_tentative = min($remaining_otd_cmo_for_this_stage, $remaining_stock_for_this_stage, $max_qty_based_on_percentage, $MaxCMO);
            }
...
// --- Stage 3: Mengisi kekurangan dari Stage 1 & 2 dengan mempertimbangkan minCarton dan skuLimit ---
// Tidak ada batasan MaxCMO lagi di stage ini.
foreach ($cmo_processed_data as &$data) {
    $mix_cust = $data['MIXCUST'];
    ...
    if ($remaining_volume_needed_for_mixcust > 0 && $current_sku_count < $skuLimit) {
        if ($remaining_otd_cmo_for_this_stage >= $minCarton && $remaining_stock_for_this_stage >= $minCarton) {
            ...
            if ($actual_qty_allocated_this_sku > 0) {
                ...
                $allocation_summary_per_mixcust[$mix_cust]['total_volume_allocated'] += $actual_volume_allocated_this_sku;
                $allocation_summary_per_mixcust[$mix_cust]['remaining_shipment_size'] -= $actual_volume_allocated_this_sku;
                $allocation_summary_per_mixcust[$mix_cust]['stage3_volume_allocated_per_mixcust'] += $actual_volume_allocated_this_sku;
